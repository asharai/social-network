...
const prop = imageComment ? images : posts;
   const commentBlock =  prop[prop?.findIndex(item=>item.id===idx)].comments.map(item=>{
       ...
       return  <Comment text={item.text} likes={item.likes} name={item.profile} time={item.date} idx={item.id} img={item.img ? item.img : 'https://i.pinimg.com/originals/51/f6/fb/51f6fb256629fc755b8870c801092942.png'}/>
